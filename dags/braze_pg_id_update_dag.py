...
def create_dynamic_dag(dag_id, job_config):
    ...
    braze_table_name = f'{table_name}_braze'
    sql_query_to_update_status = f"""
    UPDATE `dbce-c360-isl-preprod-d942.cdp_migration.{braze_table_name}`
    ...
    with dag:
        ...
        create_mapping_table_for_pgid_updates = PythonOperator(
            task_id=f"create_mapping_table_for_braze_for_{job_config['job_id']}",
            python_callable=create_mapping_table_for_segment_vs_pg_id_for_datalake_or_braze,
            op_kwargs={'mpn': job_config['mpn'], 'downstream_system': 'braze'}
        )
        ...
        trigger_alias_call = PythonOperator(
            ...
            op_kwargs={'downstream_system': 'braze', 'mpn': job_config['mpn'],
                       'table_name': "{{ task_instance.xcom_pull(task_ids=params.upstream_task_id) }}"},
            params={'upstream_task_id': f"create_mapping_table_for_braze_for_{job_config['job_id']}"}
        )
...
for job_config in job_configs:
    dag_id = f"braze_pg_id_update_for_{job_config['job_id']}"
