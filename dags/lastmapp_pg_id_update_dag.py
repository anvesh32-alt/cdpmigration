...
from utils.pgIdDownstreamUpdate import (create_mapping_table_for_segment_vs_pg_id,
                                        create_segment_vs_pg_id_file_for_lastmapp, pg_id_update_in_downstream)
...
def create_dynamic_dag(dag_id, job_config):
    ...
    with dag:
        ...
        get_or_create_pg_id_mapping_table = PythonOperator(
            task_id=f"get_or_create_pgid_mapping_table_lastmapp_for_{job_config['job_id']}",
        ...
        create_alias_file_for_pgid_updates = PythonOperator(
            task_id=f"create_alias_file_for_lastmapp_for_{job_config['job_id']}",
            python_callable=create_segment_vs_pg_id_file_for_lastmapp,
            op_kwargs={'country_code': job_config['country_code'], 'mpn': job_config['mpn']}
        )

        sftp_files_to_lastmapp = PythonOperator(
            task_id=f"sftp_files_to_lastmapp_for_{job_config['job_id']}",
            python_callable=pg_id_update_in_downstream,
            op_kwargs={'downstream_system': 'lastmapp', 'mpn': job_config['mpn'],
                       'file_path': "{{ task_instance.xcom_pull(task_ids=params.upstream_task_id) }}"},
            params={'upstream_task_id': f"create_alias_file_for_lastmapp_for_{job_config['job_id']}"}
        )

        create_export_job_for_lastmapp = PythonOperator(
            task_id=f"submit_export_job_for_lastmapp_for_{job_config['job_id']}",
            python_callable=submit_export_job_in_lytics,
            op_kwargs={'downstream_system': "lastmapp",
                       'aid_and_file_name': "{{ task_instance.xcom_pull(task_ids=params.upstream_task_id) }}"},
            params={'upstream_task_id': f"sftp_files_to_lastmapp_for_{job_config['job_id']}"}
        )

        end_process = DummyOperator(task_id="end_task")

        start_process >> get_or_create_pg_id_mapping_table >> create_alias_file_for_pgid_updates >> sftp_files_to_lastmapp \
            >> create_export_job_for_lastmapp >> end_process
...
for job_config in job_configs:
    dag_id = f"lastmapp_pg_id_update_for_{job_config['job_id']}"
