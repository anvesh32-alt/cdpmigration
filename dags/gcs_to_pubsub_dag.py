...
GCS_INPUT_BUCKET = "gs://dbce-c360-isl-preprod-d942-migration-88fakc29/migration_pipeline/{}/externalid-pipeline-output"
...
def create_dynamic_dag(dag_id, job_config, pipeline_name):
    ...
    with dag:
        ...
        dataflow_task = DataflowCreatePythonJobOperator(
            task_id=f"gcs_to_pubsub_{pipeline_name}-{job_config['job_id']}",
            py_file=DAGS_FOLDER + SRC_PATH + '/gcsToPubsub.py',
            py_options=[],
...
pipeline_names = ['registration_data', 'clickstream_data', 'clt_data', 'other_events']

# build a dag for each config and each pipeline
for job_config in job_configs:
    for pipeline_name in pipeline_names:
        dag_id = f"gcs_to_pubsub_{pipeline_name}_{job_config['job_id']}"
